...
class PostViewSet(DefaultMixin, viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        queryset = self.filter_queryset(Post.objects.filter(informante__unidade_habitacional=request.user.perfil.unidade_habitacional) | Post.objects.filter(publico=True)).order_by('-id')
